...
if (isset($_POST['title_id']) && isset($_FILES['pdf_file'])) {
    $titleId = $_POST['title_id']; // Get the research ID
    // Fetch the title name from the database using the research ID
    $stmt = $conn->prepare("SELECT email, fullname FROM srt_requests WHERE id =?");
    ...
    try {
        ...
        $mail->Body = "<h3>Request Results</h3>
                        <p>Hello $fullname, your download request for this research title has been approved</p>
        ...
        $stmt = $conn->prepare("UPDATE srt_requests SET status = 'approved' WHERE id = ?");
        $stmt->bind_param("i", $titleId);
