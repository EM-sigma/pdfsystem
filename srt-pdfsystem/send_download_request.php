...
if (isset($_POST['name2']) && isset($_POST['email2']) && isset($_POST['research_id'])) {
    ...
    $stmt = $conn->prepare("SELECT titlename FROM research_titles WHERE id =?");
    ...
    $stmt = $conn->prepare("INSERT INTO srt_requests (rtitle, email, fullname, status) VALUES (?,?,?,?)");
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $mail->addAddress('jenniferbaguls@gmail.com');  // Recipient email

            // Set email content
            $mail->isHTML(true);
            $mail->Subject = "Download Request for $title"; // Use the fetched $title
            $mail->Body = "<h3>Contact Form</h3>
                            <p><strong>Name</strong>: $name</p>
                            <p><strong>Email</strong>: $email</p>
                            <p><strong>Research Title</strong>: $title</p>"; // Include the title in the email body
